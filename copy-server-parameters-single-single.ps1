...
# Example:
#  .\copy-server-parameters-single-single.ps1 -JsonFilePath "C:\Automigration\JsonFile.json" -SourceSubscriptionId "11111111-1111-1111-1111-111111111111" -SourceResourceGroup "my-source-rg" -SingleServerName "source-server-single" -TargetSubscriptionId "11111111-1111-1111-1111-111111111111" -TargetResourceGroup "my-target-rg" -TargetSingleServerName "target-server-single"
...
# Arrays of strings - Parameters that will not be copied
...
# Arrays of strings - Parameters that will be copied to the target server
...
foreach ($item in $jsonObject) {
    ...
    if ($Overwrite -contains $item.name) {
		...
		az postgres server configuration set --name $item.name --value $item.value --subscription $TargetSubscriptionId --resource-group $TargetResourceGroup --server-name $TargetSingleServerName 
    }
}